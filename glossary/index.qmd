...
<span id="box_plot">box plot</span>
:   A plot showing the median, quartiles, and outliers of a distribution.

## C

<span id="central_limit_theorem">central limit theorem</span>
:   The theorem that the average of many independent random variables tends
...
:   A range of values, computed from data, that is likely to contain the
...
<span id="false_discovery_rate">false discovery rate</span>
:   The expected proportion of false positives among rejected hypotheses;
    controlled by the Benjamini-Hochberg procedure.
...
<span id="interquartile_range">interquartile range</span>
:   The distance between the 25th and 75th percentiles; a robust measure
    of spread.
...
:   The arithmetic average of a set of values, found by dividing the sum by
    the count.

<span id="median">median</span>
:   The middle value in a sorted set of values; robust to outliers.
...
:   A symmetric, bell-shaped distribution defined by its mean and standard
    deviation.
...
<span id="odds_ratio">odds ratio</span>
:   For binary outcomes, the ratio of the odds of an event in one group to
    the odds in another.
...
<span id="pearsons_r">Pearson's r</span>
:   A measure of linear correlation between two continuous variables,
    ranging from −1 to 1.

<span id="post_hoc_power_analysis">post-hoc power analysis</span>
:   Calculating statistical power after a study is complete, based on the
    observed effect; generally uninformative.
...
<span id="probability_distribution">probability distribution</span>
:   A function describing how likely each possible value of a variable is.
...
<span id="relative_risk">relative risk</span>
:   For binary outcomes, the ratio of the probability of an event in one
    group to the probability in another.
...
<span id="sampling_distribution">sampling distribution</span>
:   The distribution of a statistic (such as the mean) computed from many
...
<span id="spearmans_rho">Spearman's ρ</span>
:   A measure of rank-order correlation, used for monotone relationships.

<span id="standard_deviation">standard deviation</span>
:   The square root of the variance, expressing spread in the same units as
    the data.
...
<span id="type_i_error">Type I error</span>
:   A false positive: rejecting the null hypothesis when it is actually
    true.

<span id="type_ii_error">Type II error</span>
:   A false negative: failing to reject the null hypothesis when it is
    actually false.
...
:   A measure of spread equal to the average squared deviation from the
    mean.
