...
-   The {{<g mean "mean" >}} is the sum of values divided by the count;
    it is sensitive to outliers
-   The {{<g median "median" >}} is the middle value when sorted;
    use it when the distribution is skewed or when outliers are meaningful data points
    (e.g., response times, salaries)
-   The {{<g mode "mode" >}} is the most frequent value;
    it is sometimes useful for categorical data
-   {{<g variance "Variance" >}} is the average squared deviation from the mean;
    it is hard to interpret because it is in squared units
-   {{<g standard_deviation "Standard deviation" >}} (SD) is the square root of variance;
    it is in the same units as the data
-   The {{<g interquartile_range "interquartile range" >}} (IQR) is the distance between the 25th and 75th percentiles;
    it is more robust than SD when outliers are present
-   A {{<g box_plot "box plot" >}} shows median, IQR, and outliers;
    it is more informative than a bar chart with error bars for most research purposes

## Probability and Distributions

-   A {{<g probability_distribution "probability distribution" >}} describes how likely each possible value is;
    discrete distributions apply to counts, continuous distributions to measurements
-   The {{<g normal_distribution "normal distribution" >}} is symmetric and bell-shaped;
    it appears often because the {{<g central_limit_theorem "central limit theorem" >}} says that
...
-   The {{<g sampling_distribution "distribution of a sample mean" >}} (the sampling distribution)
...
-   This probability is a p-value, and small p-values are evidence against H₀
-   {{<g type_i_error "Type I error" >}} (false positive, α):
    rejecting H₀ when it is true;
    conventional threshold α = 0.05
-   {{<g type_ii_error "Type II error" >}} (false negative, β):
    failing to reject H₀ when H₁ is true;
    power = 1 − β
-   A {{<g confidence_interval "confidence interval" >}} (CI) gives a range of values consistent with the data;
...
-   Most software engineering experiments are underpowered:
    ...
    (d > 0.8) reliably
-   {{<g post_hoc_power_analysis "Post-hoc power analysis" >}} (calculating power after a null result)
...
-   {{<g bonferroni_correction "Bonferroni correction" >}}:
    use α/k as the threshold for each test;
    conservative (reduces power), but simple
-   {{<g false_discovery_rate "False discovery rate" >}} (FDR, Benjamini-Hochberg):
